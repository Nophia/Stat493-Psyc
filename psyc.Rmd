...
n.trees = seq(from=10 ,to=500, by=10)
test_error <- rep(NA, length(num.trees))
for(i in 1:length(num.trees)){
  test_error[i] <- mean((predict(rf_psyc, newdata = psyc.test, n.trees=num.trees[i]) - psyc.test$depression)^2)
}
plot(num.trees, test_error, type = 'b', xlab="number of trees", ylab = "test error")
